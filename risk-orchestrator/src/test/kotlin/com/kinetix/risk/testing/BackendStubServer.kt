package com.kinetix.risk.testing
...
/**
 * In-process Ktor HTTP server harness for risk-orchestrator acceptance tests.
 * Stands up an embedded Netty server on a random port and lets tests register
 * route handlers via the same routing DSL the production app uses.
 *
 * Equivalent in spirit to the gRPC fake-server pattern: the orchestrator's real
 * `HttpXxxServiceClient` calls a real HTTP server, so serialisation, content
 * negotiation, and HTTP wire behaviour are all exercised.
 *
 * Every received request is recorded in [recordedRequests] so tests can assert
 * what the client actually sent without resorting to mock interaction checks.
 */
