 ...
 */
class CircuitBreakerMetricsAcceptanceTest : FunSpec({

    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker is in its initial CLOSED state — GET /metrics contains the circuit_breaker_state gauge at 0.0") {
    ...
    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker opens after repeated failures — circuit is OPEN") {
        val registry = PrometheusMeterRegistry(PrometheusConfig.DEFAULT)
    ...
    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker opens after repeated failures — GET /metrics contains circuit_breaker_state gauge with value 2.0 for the circuit") {
        val registry = PrometheusMeterRegistry(PrometheusConfig.DEFAULT)
        val cb = CircuitBreaker(
            CircuitBreakerConfig(failureThreshold = 3, name = "rates-service"),
            onStateChange = CircuitBreakerMetrics.onStateChange("rates-service", registry),
    ...
    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker opens after repeated failures — GET /metrics contains circuit_breaker_transitions_total counter for CLOSED->OPEN") {
        val registry = PrometheusMeterRegistry(PrometheusConfig.DEFAULT)
        val cb = CircuitBreaker(
            CircuitBreakerConfig(failureThreshold = 3, name = "rates-service"),
            onStateChange = CircuitBreakerMetrics.onStateChange("rates-service", registry),
    ...
    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker recovers through HALF_OPEN back to CLOSED — circuit is back to CLOSED") {
    ...
    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker recovers through HALF_OPEN back to CLOSED — GET /metrics reflects CLOSED state (0.0) after recovery") {
        var now = java.time.Instant.parse("2025-01-15T10:00:00Z")
        val clock = object : java.time.Clock() {
            override fun getZone() = java.time.ZoneId.of("UTC")
            override fun withZone(zone: java.time.ZoneId?) = this
    ...
    test("a Prometheus registry with a circuit breaker wired via CircuitBreakerMetrics — the circuit breaker recovers through HALF_OPEN back to CLOSED — transition counter records all three transitions") {
        var now = java.time.Instant.parse("2025-01-15T10:00:00Z")
        val clock = object : java.time.Clock() {
            override fun getZone() = java.time.ZoneId.of("UTC")
            override fun withZone(zone: java.time.ZoneId?) = this
