...
class GovernanceAuditAcceptanceTest : FunSpec({
    ...
    test("a mix of trade and governance audit events — all are saved in sequence — hash chain spans both event types and verifies as valid") {
    ...
    test("a governance event with null trade fields — saved and read back — trade fields are null and governance fields are populated") {
        repository.save(
            governanceAuditEvent(
    ...
    test("a trade event followed by a governance event — the governance event scenarioId is tampered — verifyChain returns false") {
        repository.save(tradeAuditEvent("t-1", receivedAt = BASE))
        repository.save(
            governanceAuditEvent(
                "SCENARIO_APPROVED",
    ...
    test("governance events filtered by bookId — saved with a specific bookId — findByBookId returns only matching events") {
        repository.save(
            governanceAuditEvent(
